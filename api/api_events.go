...
type EventsApi interface {
	...
	/*
			GetMoveOnCommercialCycle Find Commercial events from unique tracking reference.
...
// not DCSA - when search is done by container reference , allow to search the last container cycle happening before the provided date
func (r ApiSearchMoveOnCommercialCycleRequest) EventDateTime(eventDateTime time.Time) ApiSearchMoveOnCommercialCycleRequest {
	r.eventDateTime = &eventDateTime
	return r
}
...
func (a *EventsApiService) SearchMoveOnCommercialCycleExecute(r ApiSearchMoveOnCommercialCycleRequest) ([]SearchMoveOnCommercialCycle200ResponseInner, *http.Response, error) {
	...
	if r.eventDateTime != nil {
		localVarQueryParams.Add("eventDateTime", parameterToString(*r.eventDateTime, ""))
	}
	if r.behalfOf != nil {
		localVarQueryParams.Add("behalfOf", parameterToString(*r.behalfOf, ""))
	}
